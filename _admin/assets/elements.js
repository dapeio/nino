...
( function(wn,dc,dE,bd) {
	...
	Nino.admin.elements = {
		...
		_openForm : function( uri ) {
			...
			Nino.admin.elements._apiCall( 'get', { type : Nino.admin.elements._currentType, uri : uri }, function( status, response ) {
		...
		_renderField : function( key, field, value ) {
			...
			if( field.type === 'image' ) {
				...
				if( field.width && field.height ) {
					...
					dimensions.className = 'admin-field-image-dimensions';
					dimensions.textContent = Nino.content.getText('/_admin/elements/label/image-target')+ ' '+ field.width+ ' × '+ field.height+ ' px';
					label.appendChild( dimensions );
				}

				const wrap = dc.createElement('div');
				wrap.className = 'admin-field-image';

				const preview = dc.createElement('img');
				preview.className = 'admin-field-image-preview';
				...
				msg.className = 'admin-field-image-msg';
		...
		_wrapWithSuffix : function( input, field ) {
			...
			row.className = 'admin-field-suffixed';
			row.appendChild( input );

			const suffix = dc.createElement('span');
			suffix.className = 'admin-field-suffix';
		...
		_renderForm : function() {
			...
			backLink.addEventListener( 'click', function( ev ) { ev.preventDefault(); Nino.admin.elements._destroyHtmlEditors(); Nino.admin.elements._showList() } );
			wrap.appendChild( backLink );
			...
			if( Nino.admin.elements._isNew === true ) {
				...
				uriHint.className = 'admin-field-hint';
			...
			if( Nino.admin.elements._isNew === false ) {
				...
				delBtn.type = 'button';
				delBtn.classList.add('admin-danger');
		...
		_save : function() {
			...
			const fields = {};
			Nino.admin.elements._globalKeys.forEach( function( key ) { fields[key] = Nino.admin.elements._readFieldByKey( key, Nino.admin.elements._currentModel[key] ) } );
			Object.assign( fields, Nino.admin.elements._localeValues[Nino.admin.elements._selectedLocale] ?? {} );
			...
			Nino.admin.elements._apiCall( 'save', {
				...
				isNew 	: Nino.admin.elements._isNew,
				fields 	: fields,
				...
				Nino.admin.elements._localeValues[Nino.admin.elements._selectedLocale] = Nino.admin.elements._localeValues[Nino.admin.elements._selectedLocale] ?? {};
				Nino.admin.elements._localeKeys.forEach( function( key ) { Nino.admin.elements._localeValues[Nino.admin.elements._selectedLocale][key] = response.element[key] ?? null } );
				...
				Nino.admin.elements._apiCall( 'list', { type : Nino.admin.elements._currentType }, function( listStatus, listResponse ) {
		...
		_uploadImage : function( key, file, hiddenInput, preview, msg, fileInput ) {
			...
			Nino.admin.elements._apiCall( 'uploadimage', {
				...
				if( status !== 200 || response === null ) {
					msg.className = 'admin-field-image-msg error';
					msg.textContent = '('+ status+ ') '+ ( ( response && response.error ) ? response.error : Nino.content.getText('/_admin/elements/error/save') );
					return;
				}
		...
		_delete : function() {
			...
			Nino.admin.elements._apiCall( 'delete', { type : Nino.admin.elements._currentType, uri : Nino.admin.elements._currentUri }, function( status, response ) {
				if( status !== 200 ) {
				...
				Nino.admin.elements._selectType( Nino.admin.elements._currentType, Nino.admin.elements._currentModel );
			} );
		},
	};
