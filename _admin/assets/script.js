	...
	wn.Nino.admin = {
		...
		router : {
			...
			isActive : function( panel ) {
				const cls = Nino.admin.router.PANEL_CLASS[panel];
				return cls !== undefined && dc.getElementById('admin-page-wrap').classList.contains( cls );
			},
		...
		exportCsv : function( filename, rows ) {
			...
			const escape = function( value ) {
				const str = Nino.admin.decodeEntities( String( value ?? '' ) );
		...
		onReady : function() {
			...
			const el = {
				...
				'navDashboard' : dc.getElementById('admin-nav-dashboard'),
				'navElements'	: dc.getElementById('admin-nav-elements'),
				'navText'			: dc.getElementById('admin-nav-text'),
				'navImages'		: dc.getElementById('admin-nav-images'),
				'navUser'			: dc.getElementById('admin-nav-user'),
				'navSubmissions' : dc.getElementById('admin-nav-submissions'),
				'navNewsletter' : dc.getElementById('admin-nav-newsletter'),
				'navLogs'			: dc.getElementById('admin-nav-logs'),
			};
			...
			const panels = {
				dashboard : [ 'show-dashboard', 	el.navDashboard, function() { Nino.admin.dashboard.showCurrent() } ],
				elements 	: [ 'show-elements', 	el.navElements, function() { Nino.admin.elements.showCurrent() } ],
				text 			: [ 'show-text', 			el.navText, 		function() { Nino.admin.text.showCurrent() } ],
				images 		: [ 'show-images', 	el.navImages, 	function() { Nino.admin.images.showCurrent() } ],
				users 		: [ 'show-user', 			el.navUser, 		function() { Nino.admin.users.showCurrent() } ],
				submissions : [ 'show-submissions', el.navSubmissions, function() { Nino.admin.submissions.showCurrent() } ],
				newsletter : [ 'show-newsletter', 	el.navNewsletter, function() { Nino.admin.newsletter.showCurrent() } ],
				logs 			: [ 'show-logs', 			el.navLogs, 		function() { Nino.admin.logs.showCurrent() } ],
			};
			...
			function selectTabFromHash() {
				const panel = Nino.admin.router.current().panel;
