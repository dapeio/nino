 ...
 *												live in this single file, same convention as
 *												_admin/Admin.php. The workbench's first-run mode. Walks a fresh checkout through
 *												environment/permission checks, assembling starting content from
 *												_admin/install/library/ and every module's own install/ unit (locales/
 *												modules/pages -> routes/templates/text/navigation, and
 *												appearance -> stylesheet/fonts + the css
 *												bundle, then separate Design/Header/Footer choices),
 *												bulk-filling the site's always-present "Personal Infos" text,
 *												creating the first workbench account(s) and finally
 ...
 *												last step is what locks the wizard back out, permanently - it
 *												stores the recovery secret and marks the project
 ...
 *												recommended) to delete the whole folder once done: `rm -rf _admin/install`.
 *
 *	@package					Dape/Nino
...
namespace Nino\Install {
	...
	/**
	 *	Nino							A compact filesystembased php framework
	 *	Install						Bootstraps the wizard: takes over the /_admin route, gates every request
	...
	class Install {
		...
		private const array MODULES = [
			...
			\Nino\Install\Accounts::class,
			\Nino\Install\Finish::class,
		];

		/**
		 *	Take over the /_admin route and its response callbacks
		 *
		...
		public static function init( array &$appData ): void {
			...
			// here let a Webpages entry mounted at /_admin replace the wizard's
			...
			// The Design step is what the Design module brings along (see
			// _designAvailable()): a delivery may ship without it, and the
			// wizard degrades to "no Design controls" rather than to a fatal

			// The wizard runs on the workbench's own route: Admin::init()
			// registers GET/POST /_admin and hands both to this class for as
			// long as the project is not set up (see Admin::isInstalled())
			$appData['/nino/http/routes']['GET://_admin'] = [
				'uri' 				=> '/_admin',
				'body'				=> '[template /_admin/install/templates/page-wizard]',
				'statusCode'	=> 200
			];
		...
		public static function handleGet( array &$appData, array &$request ): void {
			...
			$request['/nino/http/response']['body'] = '[template /_admin/install/templates/page-locked]';
		}

		/**
		 *	Fill the POST /_admin response. Every wizard api request goes
		 *	through this single route, dispatched by $_POST['action'] - same
		 *	shape as Admin::handlePost(), except there is no
		 *	separate login step: the "already installed" gate below is the
		 *	only auth the wizard has, or needs - once it trips there is no
	...
	class Checks {
		...
		private static function _extensions(): array {
			...
			// Checked separately: PharData (used for the workbench's automatic backups
			// and _admin's Restore) is only usable if the Phar extension itself is
			// loaded - phar.readonly does NOT block it (see _admin/Nino/Modules/Backups/Backups.php's
			// Backup class), so that ini setting is deliberately not checked here
			$result['Phar'] = [ 'ok' => class_exists( 'PharData' ), 'reason' => 'backup/restore archives (the Backups panel)' ];
	 ...
	 *												merged into the real /text files - see docs/setup.md's
	...
	class Setup {
		...
		public static function apiLibrary( array &$appData, array &$request ): void {
			...
			foreach( self::units() as $key => $unitDir ) {
				...
				$modules[$key] = [
					...
					// Active, or - on a project that has decided nothing yet -
					// part of the set the library recommends a site start from
					'active' 					=> ( isset( $manifest['moduleClass'] ) === true && in_array( $manifest['moduleClass'], $activeModules, true ) === true )
		...
		/**
		 *	Every module unit the wizard can offer: key => absolute unit
		 *	directory. The one place that knows where units live - Webpages
		 *	(the modules a page template pulls in) and Themes (the frame
		 *	previews' template includes and text) read it too.
		 *
		 *	A unit is a directory with a manifest.php (docs/setup.md,
		 *	"Library Format"). It travels with the module it installs: a
		 *	module ships it as install/ beside its class file, so a module
		 *	is added to a project by adding its directory, and nothing in
		 *	the wizard has to know it exists. Searched, in this order:
		 *
		 ...
		 *	point - and has to see every module a checkout ships, picked or
		 *	not.
		 *
		 *	@return 	array			key => unit directory, sorted by key
		 */
		public static function units(): array {

			$units = [];

			if( is_dir( self::LIBRARY. '/modules' ) === true )
				foreach( scandir( self::LIBRARY. '/modules' ) ?: [] as $entry )
					if( $entry[0] !== '.' )
						self::_addUnit( $units, $entry, self::LIBRARY. '/modules/'. $entry );
		...
		/**
		 *	Walk $dir for module directories carrying install/manifest.php.
		 *	A directory that is a unit is not descended into - its templates/
		 *	and text/ hold no further modules - and nothing hidden is entered
		 *
		 *	@param		array 		&$units				(reference) key => unit directory, appended to
		 *	@param		string		$dir
		 *	@param		int				$depth				Levels left to descend
		 *
		 *	@return 	void
		 */
		private static function _scanUnits( array &$units, string $dir, int $depth ): void {
			...
			foreach( scandir( $dir ) ?: [] as $entry ) {
				...
				if( $entry[0] === '.' || is_dir( $path ) === false )
					continue;

				if( is_file( $path. '/install/manifest.php' ) === true ) {
					$manifest = include $path. '/install/manifest.php';
					if( is_array( $manifest ) === true )
						self::_addUnit( $units, (string) ( $manifest['key'] ?? strtolower( $entry ) ), $path. '/install' );
					continue;
				}

				if( $depth > 1 )
					self::_scanUnits( $units, $path, $depth - 1 );
			}
		}

		/**
		 *	@param		array 		&$units				(reference) key => unit directory
		 *	@param		string		$key
		 *	@param		string		$unitDir
		 *
		 *	@return 	void
		 */
		private static function _addUnit( array &$units, string $key, string $unitDir ): void {

			if( is_file( $unitDir. '/manifest.php' ) === false )
				return;

			if( preg_match( '/^[a-z][a-z0-9-]*$/', $key ) !== 1 ) {
				trigger_error( 'Install unit in '. $unitDir. ' has no usable key ("'. $key. '" is not a slug)', E_USER_WARNING );
				return;
			}

			// The same unit reached twice - the delivered modules are scanned
			// ahead of the rest of the app dir they sit in - is not a shadow
			if( isset( $units[$key] ) === true ) {
				if( $units[$key] !== $unitDir )
					trigger_error( 'Install unit "'. $key. '" in '. $unitDir. ' is shadowed by '. $units[$key], E_USER_WARNING );
				return;
			}
		...
		public static function apiApply( array &$appData, array &$request ): void {
			...
			$modules = self::_resolveModuleRequirements( $modules, $units );
			...
			// The developer tools that ship as modules - the Design panel and
			// the Template Builder - have no unit to pick: they are active
			// whenever their directory is part of the delivery, and a class
			// listed here that a later delivery dropped is simply never
			...
			// The two roles a project starts with, written on every apply:
			// the Editor role is every content panel's permission and follows
			// the modules just picked (see \Nino\Modules\Users\Roles::defaults()), the
			// Developer role is full access. Any other role is left alone
			$appData['/nino/auth/roles'] = \Nino\Modules\Users\Roles::defaults( $appData ) + ( $appData['/nino/auth/roles'] ?? [] );

			/*	The persisted route array, never the live one: by the time any
				wizard request reaches here $appData carries this request's
				own runtime-only routes (the wizard's GET/POST pair, plus
			...
			\Nino\AppData::writeContentData( $appData, [ '/nino/locales/available', '/nino/locales/native', '/nino/modules', '/nino/auth/roles', '/nino/http/routes' ] );
		 ...
		 */
		private static function _resolveModuleRequirements( array $modules, array $units ): array {
			...
			while( $changed === true ) {
				...
				foreach( $modules as $moduleKey ) {
					...
					foreach( ( $manifest['requiresModules'] ?? [] ) as $req ) {
						...
						if( in_array( $req, $modules, true ) === true )
							continue;

						// A requirement no unit answers to is a typo in a
						// manifest, not something to apply: say so once, skip it
						if( isset( $units[$req] ) === false ) {
							if( isset( $unknown[$req] ) === false )
								trigger_error( 'Install unit "'. $moduleKey. '" requires unknown unit "'. $req. '"', E_USER_WARNING );
							$unknown[$req] = true;
							continue;
						}
	 ...
	 *												bundled. See docs/setup.md's Library format section.
	 *
	 ...
	 *
	 *												Design. The Design module owns the --nino-* colour tokens a theme
	 ...
	 *												see _designAvailable() - so a delivery without the Design module
	...
	class Themes {
		...
		// Appearance units are shared with the Design panel, which reads them
		// from here after setup (see \Nino\Modules\Design::library()). Base,
		// module and page material below the library is the wizard's alone,
		// and used only to construct synthetic previews.
		 ...
		 *
		 *	Null settings rather than an empty shape when the Design module is not part
		...
		public static function apiDesignRead( array &$appData, array &$request ): void {
			...
			\Nino\Http::ok( $request, [
				'settings' 	=> $settings,
				'choices' 	=> \Nino\Modules\Design\Tokens::choices(),
				'groups' 		=> \Nino\Modules\Design\Tokens::GROUPS,
				...
				'example' 	=> \Nino\Modules\Design\Preview::document( $appData, $settings, self::_previewMode() ),
			] );
		}

		/**
		 *	Write the design the operator settled on. A delivery without
		 *	Without the Design module there is nothing to write, and the step says so rather than failing - the
		...
		public static function apiDesignApply( array &$appData, array &$request ): void {
			...
			$data 		= \Nino\Install\Install::postData();
			$settings	= \Nino\Modules\Design\Tokens::normalize( is_array( $data['design'] ?? null ) ? $data['design'] : [] );
		 ...
		 *
		 *	Public because the Design panel composes its example page out
		...
		private static function _framePreviewMarkup( string $source ): string {
			...
			// link in the legal page. Resolved by name across every unit
			// rather than by a hard-coded list, so a frame using a new include
			// does not need this method edited
			$templateDirs = self::_unitDirs( 'templates' );

			$source = (string) preg_replace_callback( '/\[template \/templates\/([a-z0-9-]+)\]/', static function( array $include ) use ( $templateDirs ): string {

				foreach( $templateDirs as $dir )
					if( is_file( $dir. '/'. $include[1]. '.tpl' ) === true )
						return (string) file_get_contents( $dir. '/'. $include[1]. '.tpl' );
			...
			// Read through class_exists(): the module is optional, and a
			// delivery that dropped its file must still get a preview - a
			// static property read autoloads the class as unconditionally as
			// a method call would, and "Class not found" here would break
			// the wizard for a project that never wanted a navigation. Without
			// the module the frame previews a bare list in its place
			$navHtml = class_exists( '\\Nino\\Modules\\Navigation' ) === true
				? \Nino\Modules\Navigation::$html
				: [
					'li'					=> '<li><a href="[[uri]]"[[attributes]]>[[title]]</a></li>',
					'nav-burger'	=> '<div class="nino-nav-wrap [[class]]" id="[[id]]">[[content]]</div>',
					'nav-regular'	=> '<div class="nino-nav-wrap [[class]]" id="[[id]]">[[content]]</div>',
					'ul'					=> '<ul>[[content]]</ul>',
					'div'					=> '<div>[[content]]</div>',
				];
			...
			$source = (string) preg_replace_callback( '/\[navigation\b([^\]]*)\](.*?)\[\/navigation\]|\[navigation\b([^\]]*)\]/s', static function( array $nav ) use ( $items, $navHtml ): string {

				$arguments	= ( $nav[1] ?? '' ). ( $nav[3] ?? '' );
				$shell			= $navHtml[str_contains( $arguments, 'burger' ) === true ? 'nav-burger' : 'nav-regular'];
				...
				foreach( array_filter( array_map( 'trim', explode( "\n", $nav[2] ?? '' ) ) ) as $line )
					$content .= str_replace( '[[content]]', $line, $navHtml['div'] );

				$content .= str_replace( '[[content]]', $items, $navHtml['ul'] );
		...
		/**
		 *	Every directory a frame's [template ...] include or a fill's text
		 *	may come from: the base unit, then the page units, then the module
		 *	units wherever they live - Setup::units() knows
		 *
		 *	@param		string		$sub					'templates' or 'text'
		 *
		 *	@return 	array			Absolute directories, existing ones only
		 */
		private static function _unitDirs( string $sub ): array {

			$dirs = array_merge(
				[ self::INSTALL_LIBRARY. '/base' ],
				glob( self::INSTALL_LIBRARY. '/pages/*', GLOB_ONLYDIR ) ?: [],
				array_values( \Nino\Install\Setup::units() )
			);

			return array_values( array_filter(
				array_map( static fn( string $dir ): string => $dir. '/'. $sub, $dirs ),
				'is_dir'
			) );
		}
		...
		private static function _framePreviewText(): array {
			...
			foreach( [ 'global.php', 'en_US.php', '*.php' ] as $file )
				foreach( self::_unitDirs( 'text' ) as $directory )
					foreach( glob( $directory. '/'. $file ) ?: [] as $path ) {
		...
		private static function _framePreviewCss( array &$appData, array $data, string $unitDir ): string {
			...
			$css = (string) @file_get_contents( dirname( __DIR__, 2 ). '/_nino/Nino.css' );

			if( self::_designAvailable() === true ) {

				$settings = is_array( $data['design'] ?? null )
					? \Nino\Modules\Design\Tokens::normalize( $data['design'] )
					: \Nino\Modules\Design::settings( $appData );

				$css .= "\n". \Nino\Modules\Design\Tokens::css( $settings );
			}
		...
		private static function _framePreviewDocument( string $markup, string $css ): string {
			...
			return '<!doctype html><html lang="en"><head><meta charset="utf-8">'
				...
				. 'body{background-image: linear-gradient(45deg, #ffffff 25%, #fafafa 25%, #fafafa 50%, #ffffff 50%, #ffffff 75%, #fafafa 75%, #fafafa 100%);background-size: 28.28px 28.28px;}.install-frame-filler{min-height:4rem}footer{position: absolute;bottom: 0}</style>'
		 ...
		 *	maths lives in the Design module - mirroring it in javascript would be a
		 *	second implementation to keep in step.
		 *
		 *	The Design panel has an action of its own for exactly this, but it
		 *	is behind the workbench's login; during an installation there is
		 *	no account yet, so this step cannot borrow it and asks the module
		 *	directly instead
		 *
		 *	@param		array 		&$appData			(reference) Array with current app data
		 *	@param		array 		&$request			(reference) Current server request
		...
		public static function apiPreview( array &$appData, array &$request ): void {
			...
			\Nino\Http::ok( $request, [
				'settings' 	=> $settings,
				'raster' 		=> \Nino\Modules\Design\Tokens::raster( $settings ),
				'brand' 		=> \Nino\Modules\Design\Tokens::brand( $settings ),
				'example' 	=> \Nino\Modules\Design\Preview::document( $appData, $settings, self::_previewMode() ),
			] );
		}
		...
		public static function apiApply( array &$appData, array &$request ): void {
			...
			if( self::_designAvailable() === true ) {
				...
				$design = \Nino\Modules\Design\Tokens::normalize( is_array( $data['design'] ?? null ) ? $data['design'] : ( is_array( $manifest['design'] ?? null ) ? $manifest['design'] : [] ) );
		...
		/**
		 *	Whether the Design module is part of this delivery - answered
		 *	through class_exists(), which autoloads it from wherever it lives.
		 *	A release may have stripped it, so the Design step degrades to
		 *	"not offered" rather than to a fatal on a class that isn't there
		 *
		 *	@return 	bool
		 */
		private static function _designAvailable(): bool {
			return class_exists( '\Nino\Modules\Design' ) === true && class_exists( '\Nino\Modules\Design\Tokens' ) === true;
		}
		...
		private static function _themes( array &$appData ): array {
			...
			foreach( scandir( self::LIBRARY ) ?: [] as $entry ) {
				...
				// copies. Prefixed with the deploy path since the wizard's own
	...
	class Webpages {
		...
		// The workbench's runtime-owned route is not persisted in config.php,
		// so a collision check against the on-disk route array alone cannot
		// see it. A public page at this exact path would hide the workbench's
		...
		/**
		 *	List every selectable template (one per _admin/install/library/pages/&lt;key&gt;
		...
		public static function apiList( array &$appData, array &$request ): void {
			...
			// the wizard's and the modules' own bootstrap routes
			$routes 	= \Nino\Filesystem::getFileContent( $appData, '/config.php', [] )['/nino/http/routes'] ?? [];
		...
		public static function apiApply( array &$appData, array &$request ): void {
			...
			// by the wizard's and the modules' own bootstrap routes. Everything
			...
			foreach( $posted as $item ) {
				...
				// 'libraryKey' names the _admin/install/library/pages unit this
			...
			// Units live with their modules, and Setup::units() is the one
			// map of where. A page naming a unit nobody ships is a typo in
			// its manifest: said once, then skipped - same as Setup does
			$units = \Nino\Install\Setup::units();
			foreach( array_keys( array_diff_key( $requiredModuleKeys, $units ) ) as $unknownKey )
				trigger_error( 'A page template requires unknown install unit "'. $unknownKey. '"', E_USER_WARNING );
			$units = array_intersect_key( $units, $requiredModuleKeys );
		...
		/**
		 *	The _admin/install/library/pages unit one entry starts from, if it has
		 *	one at all.
		 *
		 *	@param		array 		$entry				One webpages-list entry
		 *
	 ...
	 *												and if it isn't, that's a job for the workbench's Text panel
	...
	class PersonalInfos {
		...
		// _admin/install/library/base/text/*.php - the same directory Setup's
	...
	class Accounts {
		...
		public static function actions(): array {
			return [
				'accounts/list' 		=> [ self::class, 'apiList' ],
				'accounts/create' 	=> [ self::class, 'apiCreate' ],
			];
		}
