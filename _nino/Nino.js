 ...
 *													jstext lookups. Used by the tool frontends and the public
 *													site alike, which is what decides what belongs here: this
 *													file is in the public script bundle, so anything only one
 *													audience needs is a file of its own.
 *
 *													Frontend design-system behaviors (cover, parallax, vpa,
 *													autoheight, slider, generic forms) live in Nino.ui.js;
 *													the management design system's behaviour half - the DOM
 *													primitives and the table model - in Nino.admin.js. Both
 *													extend the namespace this file creates, so both load after
 *													it.
 *
 *	@package								Dape/Nino
		...
		http : {
			...
			sendRequest : function( uri, method, callback, data, auth ) {
			  ...
			  xhr.onabort 			= responseFn;
				...
				// Auth. btoa() encodes one byte per code unit, so a password with an
				// umlaut would send its latin-1 byte while the account's hash was made
				// from the utf-8 one it was typed as - the login then fails for a
				// password that is correct - and a character above U+00FF makes btoa
				// throw outright. Encode the pair as utf-8 first, which is what the
				// server reads it back as (see \Nino\Http::_getBasicAuthCredentials())
				if( typeof auth !== 'undefined' && typeof auth.user !== 'undefined' && typeof auth.pw !== 'undefined' ) {
					let binary = '';
					new TextEncoder().encode( auth.user + ':' + auth.pw ).forEach( function( byte ) { binary += String.fromCharCode( byte ) } );
					xhr.setRequestHeader( 'Authorization', "Basic " + btoa( binary ) );
				}
