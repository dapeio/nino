...
namespace Nino\Templates {
	...
	class Templates {
		...
		public static function init( array &$appData ): void {
			...
			// Runtime-owned tool routes must win over stale persisted entries.
			// PageEditor and Install reserve /_templates for the same reason,
			// but an older config.php may still contain a collision from before
			...
			$appData['/nino/http/routes']['POST://_templates'] = [
				'uri' => '/_templates',
			];
	 ...
	 *
	 *												Settings come in seven types, all of them a two-way
	 ...
	 *												  attrtoggle  a boolean html attribute, present or absent
		...
		public static function blocks(): array {
			...
			foreach( scandir( self::LIBRARY ) ?: [] as $entry ) {
				...
				$blocks[$entry] = [
					...
					'palette' 	=> (bool) ( $manifest['palette'] ?? true ),
