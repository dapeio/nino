...
( function(wn,dc,dE,bd) {
	...
	Object.assign( wn.Nino.templates, {
		 ...
		 *	Definitions with palette:false still recognize nested markup and
		 *	drive its inspector, but stay out of this top-level insert list.
		 *	That keeps structural helpers such as table cells and modal close
		 *	buttons from crowding out useful component inserts.
		 *
		 *	@return		void
		 */
		renderPalette : function() {
			...
			Object.keys( blocks ).forEach( function( key ) {
				...
				// Recognition-only helper blocks keep nested component markup
				// editable without turning every table cell or modal close button
				// into a top-level palette entry of its own.
				if( block.palette === false )
					return;
