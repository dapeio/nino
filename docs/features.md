...
Install places files, nothing more. A newly installed feature is switched on in the panel like one copied in by hand, with everything [Activating](#activating) says. An update of an active feature is followed by that activation in one step from the panel, so the unit adds what is new and the module gets to migrate its data - see [Updating](#updating) - but `\Nino\Catalogue::install()` itself activates nothing.
