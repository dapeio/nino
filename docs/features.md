...
- **Settings** opens the form the manifest describes on a screen of its own - a button per active feature that declares any, a back link out of it - and stores it under `/nino/features` in `config.php`. Every setting is validated before any is written; an error names the setting, and nothing is saved.
