...
The panel's six actions are `features/list`, `features/activate`, `features/deactivate`, `features/settings`, `features/catalogue` and `features/install`; behind the first four stand `\Nino\Features::all()`, `activate()`, `deactivate()` and `saveSettings()`, behind the last two `\Nino\Catalogue::fetch()` and `install()`:
