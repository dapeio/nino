...
| `/nino/images/render` | `{ mode, bytes, width, height, basePath, source, filename }` | render an uploaded image another way than gd: a handler that wrote the file sets `filename` to the path below `/images/`, `false` refuses the upload, `null` passes it on |
