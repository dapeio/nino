...
Unter der Liste installiert der Block **Katalog** ein Feature ohne Kopieren: Er nennt die URL des Katalogs – `https://catalogue.getnino.dev/catalogue.json` als Standard; `/nino/catalogue/url` in der `config.php` nennt einen anderen, `''` schaltet ihn ab – und lädt ihn erst, wenn du **Katalog laden** drückst; die Workbench nimmt von sich aus nie Kontakt auf. Geladen, listet er die neueste Version jedes veröffentlichten Features, die dieses Nino ausführen kann: **Installieren** für eines, das nicht im Verzeichnis liegt, **Update** für eines, das in einer älteren Version dort liegt, „installiert“ für eines, das aktuell ist, und ausgegraut, mit dem, was es verlangt, eines, von dem keine Version passt. Eine Installation lädt das Archiv, prüft es gegen den signierten Katalog und legt das Verzeichnis an; die Aktualisierung eines aktiven Features wendet das Update im selben Schritt an. Wo `features/` nicht beschreibbar ist, verlinkt der Block stattdessen das Archiv zum Entpacken von Hand. Wie der Katalog geprüft wird, steht in [Der Katalog](features.de.md#der-katalog).
