...
Der Tab **Verfügbar** ist der Katalog: was er anbietet und diese Installation noch nicht aktuell hat. Über den Tabs schickt der Knopf **Katalog aktualisieren** der Aktionsleiste `features/catalogue`, hinter dem `\Nino\Catalogue::fetch()` und `offers()` stehen (`features/install` steht hinter `install()`), und eine Statuszeile nennt, wann der Katalog zuletzt gelesen wurde, oder dass er noch nicht gelesen wurde. Nichts wird von selbst geladen: `features/list` antwortet mit dem, was `\Nino\Catalogue::cached()` zuletzt unter `data/catalogue.php` hinterlassen hat, sodass Verfügbar sich beim Öffnen des Panels sofort füllt, ohne eine eigene Anfrage; nur Aktualisieren ruft `fetch()` erneut auf. Geladen, bietet Verfügbar **Installieren** für ein Feature, das nicht im Verzeichnis liegt, **Update** für eines, das dort in einer älteren Version liegt, und graut, mit dem, was es verlangt, eines aus, von dem keine Version passt – eines, das schon aktuell ist, erscheint hier gar nicht erst. Das Installieren lädt das Archiv, prüft es gegen den signierten Katalog, legt das Verzeichnis an und schaltet das Feature ein; eine Aktualisierung ersetzt das Verzeichnis und wendet bei einem aktiven Feature das Update in einer zweiten Anfrage an, die das Panel von sich aus stellt – die Anfrage, die die Dateien abgelegt hat, ist mit der Klasse der vorigen Version gestartet, und der Hook, den die neue Version mitbringt, kann nur in einer Anfrage laufen, die die neue lädt –, während ein Feature, das das Projekt abgeschaltet hat, abgeschaltet bleibt. Die Angebote selbst werden immer gegen die Features abgeglichen, die gerade auf der Platte liegen, also zeigt sich eine Installation auf Verfügbar auch ohne ein neues Laden. Wo `features/` nicht beschreibbar ist, verlinkt der Tab stattdessen das Archiv, zum Entpacken von Hand. Siehe [Der Katalog](#der-katalog).
...
3. Liegt ein Verzeichnis `install/` neben der Klasse, wird seine Einheit angewendet – **ohne etwas zu überschreiben**. Eine Route, deren Schlüssel die `config.php` bereits kennt, bleibt; ein Template, das `templates/` bereits hat, bleibt; eine Datei, die da ist, bleibt; ein Textschlüssel, der in `text/<locale>.php` oder `text/global.php` steht, bleibt. Nur was fehlt, wird ergänzt. Ein `config`-Standardwert der Einheit wird nur gesetzt, wo das Projekt keinen hat. Die Blacklist-Einträge der Einheit werden in `text/blacklist.php` zusammengeführt. Eine Datei, die die Einheit nicht kopieren kann – ein Ziel, das sich nicht schreiben lässt, ein Fragment, das sie nennt, aber nicht mitbringt –, weist die Aktivierung mit dieser Datei zurück, bevor etwas eingetragen oder vermerkt wird.
...
### Was Installieren tut

`\Nino\Catalogue::install()` legt Dateien ab und aktiviert nichts – das ist die Hälfte des Kernels, und sie bleibt, wie sie war. Die andere Hälfte ist `features/install` des Panels: Auf das Ablegen folgt das Aktivieren, mit allem, was [Aktivieren](#aktivieren) sagt.

In zwei von drei Fällen, und der dritte ist der Grund, sie auseinanderzuhalten. Ein Feature, das **schon aktiv war**, wird erneut aktiviert – so wird ein Update angewendet: Die Einheit ergänzt, was neu ist, und das Modul darf seine Daten migrieren, siehe [Aktualisieren](#aktualisieren). Ein Feature, das das Projekt **gar nicht hatte**, wird eingeschaltet: Wer Installieren drückt, sagt damit, dass er das Feature will, und es im Tab Inaktiv liegen zu lassen, machte aus einer Absicht zwei Klicks. Ein Feature, das **da ist und abgeschaltet wurde**, bleibt abgeschaltet – jemand hat es abgeschaltet, und eine neuere Version davon ist nicht seine Meinungsänderung. Sein Aktivieren bleibt, wo es war.

Die Antwort sagt, was davon geschah: `updated` für den ersten Fall, `activated` für den zweiten, beides falsch für den dritten. Werden die Dateien abgelegt, aber das Aktivieren scheitert – ein Manifest, das etwas verlangt, das im Verzeichnis nicht liegt, was nur ein Aktivieren wissen kann –, antwortet das Panel mit einem 400, der genau das sagt: Ein Feature, das ohne genannten Grund in der Liste steht, ist schlimmer als jede der beiden Hälften für sich.

Was es sehr wohl auflöst, sind Abhängigkeiten. `install()` ermittelt zuerst die ganze Menge – was das Feature unter `requires` nennt, was jene wiederum brauchen, und davon nur das, was das Projekt noch nicht hat –, prüft jeden Eintrag gegen diesen Kernel und legt sie dann von unten nach oben ab, sodass das eigentlich gewünschte Feature zuletzt ankommt und sich sofort einschalten lässt. Eine Abhängigkeit, die schon auf der Platte liegt, bleibt genau so, wie sie ist, in welcher Version auch immer: Eine Installation ist nicht der Moment, um etwas zu aktualisieren, das ein Projekt bewusst behalten hat. Eine Abhängigkeit, die der Katalog nicht liefern kann, weist die ganze Installation ab und nennt sie – abgelegt wird dann nichts, denn eine halbe Installation ist schlimmer als keine. Das Panel nennt in seiner Antwort, was mitgekommen ist: Wer auf einem Feature Installieren drückt und drei bekommt, soll das nicht aus dem Tab Inaktiv rekonstruieren müssen. Gesagt wird es in einem Dialog – ein installiertes Angebot ist kein Angebot mehr, seine Zeile ist also schon weg, bevor irgendwer eine Zeile darin lesen könnte.
