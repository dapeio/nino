...
Die sechs Aktionen des Panels sind `features/list`, `features/activate`, `features/deactivate`, `features/settings`, `features/catalogue` und `features/install`; hinter den ersten vieren stehen `\Nino\Features::all()`, `activate()`, `deactivate()` und `saveSettings()`, hinter den letzten beiden `\Nino\Catalogue::fetch()` und `install()`:
