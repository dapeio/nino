...
- **Einstellungen** öffnet das Formular, das das Manifest beschreibt, auf einem eigenen Bildschirm – ein Knopf je aktivem Feature, das welche erklärt, ein Zurück-Link hinaus – und speichert es unter `/nino/features` in der `config.php`. Jede Einstellung wird geprüft, bevor eine geschrieben wird; ein Fehler nennt die Einstellung, und nichts wird gespeichert.
