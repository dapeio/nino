...
Below the list, the **catalogue** block installs a feature without copying: it names the catalogue url - `https://getnino.dev/features/catalogue.json` by default; `/nino/catalogue/url` in `config.php` names another, `''` switches it off - and loads it only when you press **Load catalogue**; the workbench never contacts it on its own. Loaded, it lists the newest version of every published feature this Nino can run: **Install** for one not in the directory, **Update** for one that is there in an older version, "installed" for one that is current, and greyed out, with what it asks for, one no version of which fits. An installation downloads the archive, checks it against the signed catalogue and places the directory; updating an active feature applies the update in the same step. Where `features/` is not writable, the block links the archive to unpack by hand instead. How the catalogue is verified is in [The Catalogue](features.md#the-catalogue).
