...
**Features** sorts every feature in the `features/` directory - an installable package with a `feature.php` manifest, copied in from the catalogue [dapeio/nino-features](https://github.com/dapeio/nino-features); a checkout ships none - into three tabs, **Available**, **Inactive** and **Active**, each labelled with a count. Wherever a feature sits, it shows its name, description and version, and whatever stands in the way of switching it on: a Nino version it was not written for, a missing PHP extension, a required feature that is not there. Activating a feature applies its install unit without overwriting anything the project already has, lists its class in `/nino/modules` and records its version; deactivating removes the class and nothing else, and is refused while another active feature requires it. A feature whose directory has been replaced with a newer release shows an update, applied by activating it again. A feature that declares settings carries a **Settings** button that opens them on a screen of its own, left again through the back link; they are stored under `/nino/features` in `config.php`, and every value is validated before any is written.
