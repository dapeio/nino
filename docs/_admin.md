...
| Appearance | Link to the four-dialog `/_design` workspace | No access |
...
- remove `_admin/`, `_design/`, and `_templates/` from delivery if the interfaces are not needed in live operation.

`/_design` and `/_templates` use the same password, lock status, and session as `/_admin`. All three authenticated surfaces therefore carry the same compact **Admin / Builder / Design** bridge. It is assembled from the complete tool entry points found in the delivery: a tool that was omitted or only partially copied is not linked, and the current surface is highlighted. No separate configuration switch is involved. Without `_admin/`, neither optional tool can be used.
...
The arrows in the page list swap two page routes in `config.php`; every other route keeps its own slot. Menu entries of equal priority follow route order. Reserved paths such as `/_admin`, `/_editor`, `/_install`, `/_templates`, and `/_design` cannot be used as public pages.
...
Any save through `/_admin`, `/_editor`, `/_design`, `/_templates` or `/_install` drops the whole cache immediately — a single textfill or appearance change can change every page, so there is no smaller useful unit. The lifetime above only bounds how long a page survives without any edit at all.
