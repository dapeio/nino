...
**Links:**
[README](../README.md) · [Design-Handbook](design.md) · [Developer-Handbook](development.md) · [_dev-Handbook](_dev.md) · [Security Policy](../SECURITY.md) · [Changelog](../CHANGELOG.md)

Precise instructions for the day-to-day maintenance of the website via `/_admin` — no programming knowledge required.
For technical background: `docs/development.md`.

### Signing in

Sign in at `/_admin` with an email address and password. After too
...
model, created via `/_dev` → "Element Types", see section 11).
...
(section 8) — an accidental deletion can only be undone via `/_dev` →
"Restore", not from within `/_admin` itself.
...
by the developer (created via `/_dev` → "Images", see section 11). An
...
developers via `/_dev` and is not possible through `/_admin` (see
section 11) — the separation ensures that creating a first account
never depends on an already existing `/_admin` account.
...
`/_dev` permissions), only `/_dev` helps — contact the developer, who
has access to the `_dev` password.
...
Read-only; no deleting or editing via `/_admin`. 90 days of history,
...
when) — every mutating action via `/_admin` (saving, deleting,
...
## 2. The developer area (_dev)

### a) Automatic backups

Once a day, triggered by the first login of the day (not as a separate
cron job — it runs in the background of the first `/_admin` request a
signed-in user makes that day), an encrypted backup is created of all
content editable via `/_admin` (elements, texts, images, user
...
for restoration via `/_dev` (section 9) and cannot be viewed directly
...
### b) Backup restoration (via `/_dev` only)

The state of any day within the last 14 days can be restored via
`/_dev` → "Restore". The current (possibly faulty) state is
...
`/_dev` has its own password, completely independent of the `/_admin`
accounts (known only to the developer) — which is why restoration still
works even when the `/_admin` user accounts are precisely the damaged
part.

### c) First access & passwords (`/_dev`)

Without a single existing `/_admin` account there is no way to sign in
there — the very first access therefore always comes through `/_dev` →
...
`/_admin`. Creating new `/_admin` accounts or deleting existing ones
remains a `/_dev` task permanently thereafter (see section 5).

`/_dev` itself has no form login with an email — only a single,
project-wide password, set via the command line (`php _dev/Dev.php
<password>`, with the resulting hash entered into the `PASSWORD_HASH`
constant in `_dev/Dev.php`). The placeholder hash shipped with the
project deliberately matches no real password — your own must be set
before first real use, otherwise `/_dev` is unusable for anyone, not
even for the developer.

## d) Further `/_dev` areas (brief overview for operators)
...
- **Element Types** — defines the field model for element types (which
  fields an "offering" or "team member" has), before `/_admin` →
  "Elements" (section 2) can maintain data with them.
- **Images** — creates image slots (fixed positions with a target
  format) before `/_admin` → "Images" (section 4) can fill them; also
...
- **Texts** — creates new text keys, renames them, and contains the
  ...
  not yet defined — before `/_admin` → "Texts" (section 3) can fill
  them.
...
- Do not share passwords, not even within the team — every `/_admin`
