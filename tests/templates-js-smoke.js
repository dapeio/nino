...
blocks.setAll( {
	...
	'button' : {
		...
		match : { tags : [ 'a', 'button' ], classes : [ 'ui-btn' ], classesAny : [], attrs : {}, not : [ 'js-modal-trigger' ] },
		settings : { href : { type : 'attr', attr : 'href' } },
	},
	'modal-trigger' : {
		key : 'modal-trigger', category : 'Interactive', tag : 'link', name : 'Modal Trigger',
		match : { tags : [ 'a', 'button' ], classes : [ 'js-modal-trigger' ], classesAny : [], attrs : {}, not : [] },
		settings : { target : { type : 'attr', attr : 'data-modal-target' } },
	},
	'boolean-attr' : {
		key : 'boolean-attr', category : 'Test', tag : 'wrap', name : 'Boolean Attribute',
		match : { tags : [ 'details' ], classes : [], classesAny : [], attrs : {}, not : [] },
		settings : { open : { type : 'attrtoggle', attr : 'open' } },
	},
	'sc-elements' : {
...
check( 'a behavioural button wins even when ui-btn styles it too', blocks.match( el( 'button', [ 'ui-btn', 'js-modal-trigger' ] ) ) === 'modal-trigger' );
...
const booleanAttr = el( 'details', [], { open : '' } );
check( 'reads a present boolean attribute as enabled', blocks.read( booleanAttr, 'boolean-attr' ).open === true );
check( 'reads an absent boolean attribute as disabled', blocks.read( el( 'details', [] ), 'boolean-attr' ).open === false );
...
blocks.write( booleanAttr, 'boolean-attr', 'open', false );
check( 'an attrtoggle removes its boolean attribute when disabled', ( 'open' in booleanAttr.attrs ) === false );
blocks.write( booleanAttr, 'boolean-attr', 'open', true );
check( '...and restores it as a bare boolean value when enabled', booleanAttr.attrs.open === '' );
