 ...
 *	editor-smoke.php	Dependency-free smoke test for the editor backend
 *											(_editor/Editor.php) - the Text editor's blacklist
...
\Nino\Editor\Text::apiKeys( $appData, $request );
...
// --- Editor::sessionLocale / apiSetLocale --------------------------------------

echo "Editor::sessionLocale / apiSetLocale\n";

check( 'sessionLocale defaults to the native locale before anything is chosen', \Nino\Editor\Editor::sessionLocale( $appData ) === 'de_DE' );

$request = [ '/nino/http/response' => [ 'statusCode' => 200 ] ];
$_POST['data'] = json_encode( [ 'locale' => 'not-a-locale' ] );
\Nino\Editor\Editor::apiSetLocale( $appData, $request );
check( 'apiSetLocale rejects a locale that is not available', $request['/nino/http/response']['statusCode'] === 400 );
check( 'sessionLocale is unaffected by the rejected attempt', \Nino\Editor\Editor::sessionLocale( $appData ) === 'de_DE' );
...
check( 'sessionLocale now returns the newly chosen locale', \Nino\Editor\Editor::sessionLocale( $appData ) === 'en_US' );

$request = [ '/nino/http/response' => [ 'statusCode' => 200 ] ];
\Nino\Editor\Elements::apiTypes( $appData, $request );
...
/**
 *	Call Text::apiSaveBatch() with a single item, like the real POST /_editor dispatch does
 *
 *	@param		array 		&$appData
...
function saveText( array &$appData, array $data ): array {
	...
	\Nino\Editor\Text::apiSaveBatch( $appData, $request );
...
\Nino\Editor\Elements::apiDelete( $appData, $titledDeleteRequest );
...
function callUploadSlotImage( array &$appData, string $uri ): array {
	...
	$_FILES['file'] = [ 'tmp_name' => $path, 'error' => UPLOAD_ERR_OK, 'name' => 'test.jpg', 'size' => filesize( $path ) ];
	\Nino\Editor\Images::apiUpload( $appData, $request );
	@unlink( $path );
	return [ $request['/nino/http/response']['statusCode'], $request['/nino/http/response']['body'] ];
}

$listRequest = [ '/nino/http/response' => [ 'statusCode' => 200 ] ];
\Nino\Editor\Images::apiList( $appData, $listRequest );
...
function callUsers( array &$appData, string $method, array $data = [] ): array {
	$request = [ '/nino/http/response' => [ 'statusCode' => 200 ] ];
	$_POST['data'] = json_encode( $data );
	\Nino\Editor\Users::{$method}( $appData, $request );
	return [ $request['/nino/http/response']['statusCode'], $request['/nino/http/response']['body'] ];
}
...
// by the "Editor::guardPerm" section below, so this must not touch it
...
[ $status ] = callUsers( $appData, 'apiSetPermissions', [ 'username' => 'permtest@example.com', 'perms' => [ \Nino\Editor\Elements::MANAGE_PERM ] ] );
...
[ $status, $body ] = callUsers( $appData, 'apiSetPermissions', [ 'username' => 'permtest@example.com', 'perms' => [ \Nino\Editor\Elements::MANAGE_PERM, 'not/a/real/perm' ] ] );
check( 'apiSetPermissions succeeds and whitelists away an unknown perm string', $status === 200 && $body['perms'] === [ \Nino\Editor\Elements::MANAGE_PERM ] );
check( 'the perm actually persisted to the user record', \Nino\Auth::getUser( $appData, 'permtest@example.com' )['perms'] === [ \Nino\Editor\Elements::MANAGE_PERM ] );
...
// --- Editor::guardPerm - per-module permissions ------------------------------

echo "Editor::guardPerm - per-module permissions (Elements/Text/Images/Submissions/Newsletter/Logs)\n";
...
\Nino\Auth::insertUser( $appData, 'contenteditor@example.com', 'editor password', [ \Nino\Editor\Elements::MANAGE_PERM ] );
...
echo "Backup::maybeRun (triggered from Editor::guard on every authenticated request)\n";

// Every earlier guarded call in this whole file (Text::apiSaveBatch etc., way
// above) already ran Backup::maybeRun() at least once, since Editor::guard()
...
// --- Logs::record via Editor::handlePost()'s per-action hook + login hook ---

echo "Admin activity log (Logs::record via handlePost()'s per-action hook + login hook)\n";

/**
 *	Dispatch one POST /_editor action through the real Editor::handlePost()
...
function callAdminPost( array &$appData, string $action, array $data = [] ): array {
	...
	$_POST['action'] = $action;
	$_POST['data'] = json_encode( $data );
	\Nino\Editor\Editor::handlePost( $appData, $request );
	return [ $request['/nino/http/response']['statusCode'], $request['/nino/http/response']['body'] ];
}
...
// Editor::guard() has already been called many times by earlier sections
...
// --- Login hook: Editor::guard()/handleGet() log "Login" the first time a
...
// through _editor/index.php at all (see Editor::_logLoginOnce()'s docblock)
...
check( 'lastBackup is today\'s date (Backup::maybeRun already ran via Editor::guard() many times above)', $body['lastBackup'] === date( 'Y-m-d' ) );
