...
namespace Nino\Modules {
	 ...
	 *												the Submissions panel (in addition to the mail itself) - see
	 *												Admin\Submissions in _admin/Editor.php, which reads this
	...
	class Form {
		...
		/**
		 *	Validate and send the contact form, then record it for the Submissions panel -
		...
		public static function callbackResponse( array &$appData, array &$request ): void {
			...
			// Recipient/subject are Text values ([[/form/...]], editable in the Text panel) -
			...
			// Stored escaped, same as before: the Submissions panel decodes
			// these entities again on render (see Admin/Admin.php and assets/admin.js,
		 ...
		 *	Admin\Submissions in _admin/Editor.php (same shape as
		 ...
		 *	the workbench deployed at all) rather than needing a decoder. Lives
		 *	in /data, not under _admin/, since this is public-site kernel
		 *	data the Submissions panel merely happens to read - same fixed-path reasoning
