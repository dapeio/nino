...
- 76-definition block library as one directory per block (`_templates/library/`), covering the `Nino.css` component catalogue with recognition-only helpers kept out of the insert palette
- Grid widths and spacing drawn to scale; everything else a labelled box, see `docs/_templates.md`
- Generated settings for classes, text, tags and attributes, including checkbox-safe native boolean attributes (`required`, `open`, `controls`, …)
