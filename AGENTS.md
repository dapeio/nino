...
| `content/` | The project's private half — never served, only read by PHP: `config.php`, `templates/`, `text/`, `elements/`, `data/`, plus `.auth/` (the `/_admin` hash, login throttle, backup key), `.logs/` and `.backups/`. Reached through `Filesystem::path()` (which resolves `PRIVATE_DIRS` against it) or the virtual `Filesystem::CONTENT_DIR` prefix; moved by `NINO_CONTENT_DIR`. Never write project state into a tool folder or into `config.php` — the first breaks updates, the second is rolled back by a Restore |
