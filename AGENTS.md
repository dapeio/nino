...
before copying patterns from memory or from another framework. The six
...
| Path | Ownership |
| --- | --- |
| `_nino/Nino.php` | Boot: `\Nino\init()`, `request()`, `output()` and the autoloader |
| `_nino/Nino/<Class>/<Class>.php` | The kernel classes and public core APIs: AppData, Auth, Callbacks, Csrf, Filesystem, Backup, RotatingLog, Elements, Html, Http, Images, Locales, Text, Mail, Modules, Runtime |
...
| `docs/recipes/` | The six extension recipes of this guide, English only |
...
The six extension recipes live under `docs/recipes/`. They are part of this
...
| [Add an installer module package](docs/recipes/installer-package.md) | a feature the setup wizard can select, copy and configure |
