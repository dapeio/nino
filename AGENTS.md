...
| the Template Builder | Not here since 1.2: extracted into the feature [`features/Templates`](https://github.com/dapeio/nino-features/blob/main/features/Templates/README.md) of the catalogue [dapeio/nino-features](https://github.com/dapeio/nino-features), with its section preset library, its two tests and its manual. A project that installed it carries it below `features/`, where the autoloader serves `\Nino\Modules\Templates` from - the kernel root resolves first, so a Nino that still shipped the module would shadow it. That is one of two reasons the feature names `^1.3`; the other is that its own manifest carries a sectioned `manual`, which only a kernel newer than the `v1.2.0-beta` tag can read |
