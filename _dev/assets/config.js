 ...
 *													json - see _dev/Dev.php's Config class docblock for why
...
( function(wn,dc,dE,bd) {
	...
	Nino.dev.config = {
		...
		init : function() {
			...
			Nino.dev.config._apiCall( 'list', {}, function( status, response ) {
				if( status !== 200 || response === null )
					return Nino.dev.config._showError( dc.getElementById('config-list'), status, response );

				Nino.dev.config._values = response.values;
				Nino.dev.config._renderList();
				Nino.dev.config._showList();
				Nino.dev.config._ready = true;
			} );
		},
		...
		showCurrent : function() {
			...
			if( Nino.dev.config._ready === false )
				return Nino.dev.config.init();

			if( dc.getElementById('config-form').classList.contains('dev-hidden') === false )
				return Nino.dev.config._showForm();

			Nino.dev.config.init();
		},
		...
		_showList : function() {
			dc.getElementById('config-list').classList.remove('dev-hidden');
			dc.getElementById('config-form').classList.add('dev-hidden');
		},

		_showForm : function() {
			dc.getElementById('config-list').classList.add('dev-hidden');
			dc.getElementById('config-form').classList.remove('dev-hidden');
		},
		...
		_renderList : function() {
			...
			Object.keys( Nino.dev.config._values ).forEach( function( key ) {
				const li 		= dc.createElement('li');
				const link	= dc.createElement('a');
				link.href = '#';
				link.textContent = key;
				link.addEventListener( 'click', function( ev ) { ev.preventDefault(); Nino.dev.config._openForm( key ) } );
				li.appendChild( link );
				ul.appendChild( li );
			} );
			wrap.appendChild( ul );
		...
		_openForm : function( key ) {
			...
			Nino.dev.config._currentKey = key;
			...
			backLink.addEventListener( 'click', function( ev ) { ev.preventDefault(); Nino.dev.config._showList() } );
			...
			textarea.value = Nino.dev.config._values[key] ?? '';
			...
			form.addEventListener( 'submit', function( ev ) { ev.preventDefault(); Nino.dev.config._save() } );

			wrap.appendChild( form );
			Nino.dev.config._showForm();
		},
		...
		_save : function() {
			...
			Nino.dev.config._apiCall( 'save', { key : Nino.dev.config._currentKey, value : value }, function( status, response ) {
				if( status !== 200 || response === null ) {
	...
	Nino.events.bindCallback( 'ready', Nino.dev.config.init );
