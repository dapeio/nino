 ...
 *													backups _admin/Admin.php's Backup class creates, and
...
( function(wn,dc,dE,bd) {
	...
	Nino.dev.restore = {
		...
		init : function() {
			...
			Nino.dev.restore._apiCall( 'list', {}, function( status, response ) {
				if( status !== 200 || response === null )
					return Nino.dev.restore._showError( status, response );

				Nino.dev.restore._renderList( response.dates );
				Nino.dev.restore._ready = true;
			} );
		},
		...
		showCurrent : function() {
			if( Nino.dev.restore._ready === false )
				Nino.dev.restore.init();
		},
		...
		_renderList : function( dates ) {
			...
			dates.forEach( function( date ) {
				...
				btn.addEventListener( 'click', function() { Nino.dev.restore._confirmRestore( date ) } );
				li.appendChild( btn );
		 ...
		 *	overwritten (see Restore::apiRestore() in _dev/Dev.php)
		 *
		 *	@param		{string}		date
		 *
		 *	@return		void
		...
		_confirmRestore : function( date ) {
			...
			Nino.dev.restore._apiCall( 'restore', { date : date }, function( status, response ) {
	...
	Nino.events.bindCallback( 'ready', Nino.dev.restore.init );
