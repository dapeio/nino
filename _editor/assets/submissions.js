...
( function(wn,dc,dE,bd) {
	...
	Nino.editor.submissions = {
		...
		init : function() {
			...
			Nino.editor.submissions._apiCall( 'list', {}, function( status, response ) {
				if( status !== 200 || response === null )
					return Nino.editor.submissions._showError( status, response );

				Nino.editor.submissions._renderList( response.entries );
			} );
		},
		...
		_renderEntry : function( entry ) {
			...
			if( entry.cat ) {
				...
				cat.textContent = Nino.editor.decodeEntities( entry.cat );
			...
			mailLink.textContent = Nino.editor.decodeEntities( entry.name ?? '' )+ ' <'+ Nino.editor.decodeEntities( entry.email ?? '' )+ '>';
			...
			message.textContent = Nino.editor.decodeEntities( entry.message ?? '' );
			...
			li.addEventListener( 'keydown', function( ev ) {
				if( ev.key !== 'Enter' && ev.key !== ' ' )
					return;
				ev.preventDefault();
				Nino.editor.submissions._toggleEntry.call( li );
			} );
